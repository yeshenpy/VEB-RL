...
def run_exp(args):
    ...
    wandb.run.name =  "GA_VEB_version_"+str(args.frac)+"_Pop_size_"+ str(pop_size) + "_"+ str(elite_size)+"_Update_EA_target_"+str(args.EA_target_update_freq)+"_Frac_" +"_"+str(args.frac) + "_"+ str(args.rl_to_ea_synch_period)+"_"+str(hyperparam_config['env']) + '_' + hyperparam_config['alg_name']
    wandb.run.save()
    ...
    # Init RL agent
    evolution = SSNE(pop_size,args.frac, 0.1)
    Pop = []
    for i in range(pop_size):
        gen = Genome(state_shape=state_shape, num_actions=num_actions, device=device,learning_rate=args.lr, hard_replacement_interval=args.hard_replacement_interval)# #epsilon_decay_steps=int(100000/6.0))
        Pop.append(gen)
    ...
    while global_step_count < MAX_TOTAL_STEPS:
        ...
        if global_step_count - previous_print_steps >= args.eval_interval:
            ...
            ea_eval_return, ea_eval_step = evaluate_policy(Pop[elite_index[0]], eval_env=env4eval, eval_episodes=10, env_name=args.env)
            ...
            wandb.log({'elite': evolution.selection_stats['elite']/evolution.selection_stats['total']}, step=global_step_count)
            wandb.log({'selected': evolution.selection_stats['selected']/evolution.selection_stats['total']}, step=global_step_count)
            wandb.log({'discarded': evolution.selection_stats['discarded']/evolution.selection_stats['total']}, step=global_step_count)
        ...
        if ep_num % args.rl_to_ea_synch_period == 0 and EA_generation > 0:
            ...
            while replace_index in  best_agents:
                replace_index = (replace_index + 1) % len(Pop)

            rl_to_evo(agent, Pop[replace_index])

            evolution.rl_policy = replace_index
